    ...
    class BarclaycardSmartpayGateway < Gateway
      ...
      def credit(money, creditcard, options = {})
        ...
        if options[:third_party_payout]
          ...
          MultiResponse.run do |r|
            r.process { commit('storeDetailAndSubmitThirdParty', post, @options[:store_payout_account], @options[:store_payout_password]) }
            r.process { commit('confirmThirdParty', post, @options[:review_payout_account], @options[:review_payout_password]) }
          end
        else
      ...
      def commit(action, post, account = 'ws', password = @options[:password])
        puts "OKAY BOYS WE'RE GOING WITH #{account} ===> #{password}"
